...
extension IteratorProtocol where Element == UInt8 {
  ...
  /**
   ...
   - Parameter maxLength: The maximum number of bytes to return. maxLength must be greater than or equal to zero.
   */
  public mutating func next(until predicate: (UInt8) -> Bool) -> (values: [UInt8], didReachEnd: Bool) {
    var bytes = [UInt8]()
    while let byte = next() {
      if predicate(byte) {
        return (values: bytes, didReachEnd: false)
      }
      bytes.append(byte)
    }
    return (values: bytes, didReachEnd: true)
  }
}
